 ...
 * _    3、相对于 Stack时间耗费更少
 * _    4、强制严格按照栈定义来操作栈带来的好处：功能完整且单一，避免了使用时对某些函数混淆或者其他
 * _
 * _    5、强制严格按照栈定义来操作栈带来的坏处：只能栈尾删除，栈尾增加
 * _    6、劣势2：获取对象引用需定位两次（elementArray[index1][index2]），a、定位到index1 获取引用；b、然后根据index2和之前的引用定位获得对象地址
 * 
